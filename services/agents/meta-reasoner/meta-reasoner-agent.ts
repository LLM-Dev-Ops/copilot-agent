 ...
 */
export class MetaReasonerAgent implements BaseAgent<MetaReasonerInput, MetaReasonerOutput> {
  ...
  validateInput(input: unknown): MetaReasonerInput {
    return MetaReasonerInputSchema.parse(input);
  }
   ...
   */
  async invoke(input: MetaReasonerInput, executionRef: string): Promise<AgentResult> {
    ...
    try {
      ...
      // Validate output
      const validatedOutput = MetaReasonerOutputSchema.parse(output);
   ...
   */
  private analyzeReasoning(input: MetaReasonerInput): MetaReasonerOutput {
   ...
   */
  private detectContradictions(traces: ReasoningTrace[]): Contradiction[] {
    const contradictions: Contradiction[] = [];
    let contradictionCount = 0;

    // Group traces by decision type for comparison
    const tracesByType = new Map<string, ReasoningTrace[]>();
    ...
    for (const [decisionType, typeTraces] of tracesByType) {
      if (typeTraces.length < 2) continue;
    ...
    const tracesByAgent = new Map<string, ReasoningTrace[]>();
    for (const trace of traces) {
    ...
    for (const [agentId, agentTraces] of tracesByAgent) {
      if (agentTraces.length < 2) continue;

      // Sort by timestamp
      const sorted = [...agentTraces].sort(
   ...
   */
  private findConflictingConstraints(set1: Set<string>, set2: Set<string>): string[] {
  ...
  private assessConfidenceCalibration(
    ...
    for (const [agentId, agentTraces] of tracesByAgent) {
      ...
      let assessment: ConfidenceCalibration['assessment'];
      let calibrationGap: number | undefined;
      ...
      if (expectedAccuracy !== undefined) {
        ...
        if (Math.abs(calibrationGap) <= 0.1) {
          ...
          recommendations.push(
            `Consider reducing confidence by ~${(calibrationGap * 100).toFixed(0)}%`,
            'Review historical accuracy data for calibration'
          );
        } else {
          calibrationScore = Math.max(0, 1 + calibrationGap);
          assessment = 'underconfident';
          recommendations.push(
            `Consider increasing confidence by ~${(Math.abs(calibrationGap) * 100).toFixed(0)}%`,
            'Agent may be underestimating its capabilities'
          );
        }
      } else {
        ...
        if (confidenceVariance > 0.1) {
          ...
          recommendations.push(
            'Confidence varies significantly across invocations',
            'Consider implementing more stable confidence estimation'
          );
        } else {
          calibrationScore = 0.7; // Default moderate score without historical data
          assessment = 'insufficient_data';
          recommendations.push(
            'Historical accuracy data needed for proper calibration assessment',
            'Track prediction outcomes to build accuracy baseline'
          );
        }
      }
   ...
   */
  private identifySystemicIssues(traces: ReasoningTrace[]): SystemicIssue[] {
    ...
    for (const [decisionType, constraints] of constraintSets) {
      ...
      const uniqueConstraintCombinations = new Set(
        tracesOfType.map(t => JSON.stringify([...t.constraints_applied].sort()))
      );
  ...
  private calculateQualityMetrics(
    ...
    const overallScore = (
      ...
      clarityScore * 0.25 +
      constraintAdherenceScore * 0.2
    ) * (1 - Math.min(systemicIssues.length * 0.05, 0.3));
  ...
  private generateKeyFindings(
    ...
    calibrations: ConfidenceCalibration[],
    systemicIssues: SystemicIssue[],
    qualityMetrics: ReasoningQualityMetrics
  ): MetaReasonerOutput['key_findings'] {
    const findings: MetaReasonerOutput['key_findings'] = [];
    ...
    // Calibration issues
    const poorlyCalibrated = calibrations.filter(
      c => c.assessment === 'overconfident' || c.assessment === 'underconfident'
    );
    ...
    // Systemic issues
    const severeSystems = systemicIssues.filter(
   ...
   */
  private extractAssumptions(input: MetaReasonerInput): string[] {
   ...
   */
  private calculateConfidence(output: MetaReasonerOutput, input: MetaReasonerInput): number {
   ...
   */
  private getAppliedConstraints(input: MetaReasonerInput): string[] {
    const constraints = [
      'read_only_analysis',
      'no_output_override',
