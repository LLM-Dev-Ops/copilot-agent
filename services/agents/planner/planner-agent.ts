...
export class PlannerAgent implements BaseAgent<PlannerInput, PlannerOutput> {
  ...
  validateInput(input: unknown): PlannerInput {
    return PlannerInputSchema.parse(input);
  }
   ...
   */
  async invoke(input: PlannerInput, executionRef: string): Promise<AgentResult> {
    ...
    try {
      ...
      // Validate output
      const validatedOutput = PlannerOutputSchema.parse(output);
   ...
   */
  private decomposeObjective(input: PlannerInput): PlanStep[] {
    ...
    steps.push(this.createStep(
      ...
      'Analyze Requirements',
      'Analyze and validate the requirements from the objective',
      0,
      [],
    ...
    steps.push(this.createStep(
      ...
      1,
      [{ depends_on: 'analyze-requirements', type: 'blocking' }],
      ['design', 'architecture'],
      'high'
    ));
    ...
    if (this.containsAny(objective, ['build', 'create', 'implement', 'develop'])) {
      steps.push(this.createStep(
        'plan-implementation',
        'Plan Implementation',
        'Define implementation steps and technical approach',
        2,
        [{ depends_on: 'design-solution', type: 'blocking' }],
    ...
    if (this.containsAny(objective, ['test', 'validate', 'verify', 'check'])) {
      steps.push(this.createStep(
        'plan-validation',
        'Plan Validation',
        'Define validation criteria and test approach',
        steps.length,
        [{ depends_on: steps[steps.length - 1]?.step_id || 'design-solution', type: 'data' }],
        ['validation', 'testing'],
        'medium'
      ));
    }
    ...
    if (this.containsAny(objective, ['integrate', 'connect', 'api', 'service'])) {
      steps.push(this.createStep(
        'plan-integration',
        'Plan Integration',
        'Define integration points and interface contracts',
        steps.length,
        [{ depends_on: 'design-solution', type: 'data' }],
    ...
    steps.push(this.createStep(
      'plan-documentation',
      'Plan Documentation',
      'Define documentation requirements and structure',
      steps.length,
      [{ depends_on: 'design-solution', type: 'data' }],
      ['documentation'],
      'low',
    ...
    steps.push(this.createStep(
      ...
      'Define Completion Criteria',
      'Establish clear criteria for plan completion',
      steps.length,
      steps.slice(0, -1).map(s => ({ depends_on: s.step_id, type: 'data' as const })),
      ['completion', 'criteria'],
      'high'
    ));
   ...
   */
  private buildDependencyGraph(steps: PlanStep[]): Record<string, string[]> {
  ...
  private identifyParallelGroups(
    ...
    while (completed.size < steps.length) {
      ...
      if (currentGroup.length === 0) break; // Circular dependency protection
   ...
   */
  private resequenceSteps(steps: PlanStep[]): PlanStep[] {
  ...
  private identifyRisks(
    steps: PlanStep[],
    input: PlannerInput
  ): Array<{ description: string; severity: 'low' | 'medium' | 'high'; affected_steps: string[] }> {
    const risks: Array<{ description: string; severity: 'low' | 'medium' | 'high'; affected_steps: string[] }> = [];
   ...
   */
  private extractAssumptions(input: PlannerInput): string[] {
   ...
   */
  private summarizeObjective(objective: string): string {
   ...
   */
  private calculateConfidence(output: PlannerOutput): number {
   ...
   */
  private getAppliedConstraints(input: PlannerInput): string[] {
    const constraints = [
      'read_only_analysis',
      'no_execution',
      'no_agent_assignment',
